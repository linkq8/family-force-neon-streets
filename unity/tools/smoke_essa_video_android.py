...
OUT=ROOT/'unity/Builds'/('EssaClear222' if clear else 'EssaVideo221')
...
tap(1073,620);time.sleep(.8 if clear else 1.5)
tap(953,620);time.sleep(.9 if clear else 1.7)
...
assert 'NullReferenceException' not in logs and 'Missing Essa222 atlas' not in logs
version='0.5.4-essa-clear-fast' if clear else '0.5.3-essa-video'
assert ('versionName='+version) in call('shell','dumpsys','package','com.familyforce.neonstreets.unityprototype').decode()
report=dict(status='SMOKE_COMMANDS_PASS',installedVersion=version,touchCommands=['left','right','punch','kick'],newIdleAndWalkLoaded=True,missingAtlasOrNullExceptions=False,visualReviewRequired=True,physicalDeviceTested=False)
