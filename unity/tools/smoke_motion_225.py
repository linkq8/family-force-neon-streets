...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/('Builds/Repairs232' if '--repairs' in sys.argv else 'Builds/Motion225')
...
if '--repairs' in sys.argv:
    call('shell','input','tap','833','620');time.sleep(.8)
...
(OUT/'android-render-validation.json').write_text(json.dumps(dict(status='PASS',installedVersion='0.5.10-repairs' if '--repairs' in sys.argv else '0.5.6-smooth',walkFrames=35,walkSamples=walk,measurement='camera render submissions; video recorded separately',physicalUserDevice=False),indent=2)+'\n')
