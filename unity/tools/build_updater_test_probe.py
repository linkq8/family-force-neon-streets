...
import subprocess, tempfile, zipfile, os, sys
...
with tempfile.TemporaryDirectory(prefix='ff-update-probe-') as tmp:
    ...
    run(JDK/'javac','-source','8','-target','8','-classpath',ANDROID,'-d',classes,
        *(ROOT/'android-updater/src').glob('**/*.java'),ROOT/'android-updater/tests/PolicyTests.java',*([ROOT/'android-updater/tests/InstallProbe232.java'] if repairs else []))
    ...
    if repairs:
        manifest=manifest.replace('android:versionCode="5"','android:versionCode="10"').replace('android:versionName="0.5.4-updater-QA"','android:versionName="0.5.9-updater-QA"')
        manifest=manifest.replace('<application android:debuggable', '<instrumentation android:name="com.familyforce.updates.InstallProbe232" android:targetPackage="com.familyforce.neonstreets.unityprototype"/><application android:debuggable')
    ...
    apk=OUT/('LOCAL-ONLY-UpdaterProbe-0.5.9.apk' if repairs else 'LOCAL-ONLY-UpdaterProbe-0.5.4.apk')
