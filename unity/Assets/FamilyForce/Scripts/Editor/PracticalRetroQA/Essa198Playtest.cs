...
namespace FamilyForce.Unity.Editor
{
    ...
    [InitializeOnLoad]
    public static class Essa198Playtest
    {
        const string Running = "FF_Essa198_QA";
        static readonly string Output = Path.GetFullPath("../art-pipeline/builds/Essa/practical-198/unity-test");
        ...
        static void Tick()
        {
            ...
            try
            {
                ...
                if (hero == null)
                {
                    ...
                    foreach (string action in new[]{"idle","walk","punch","kick","heavy_punch","heavy_kick","jump","special","link","hurt","knockdown"})
                        foreach (var sprite in CharacterAtlasCatalog.LoadClip("Essa",action))
                            Require(sprite.name.StartsWith("Essa198_"), "Unexpected legacy identity swap: " + action);
                ...
                {
                    Require(renderer.sprite.name == "Essa198_guard", "Idle did not restore practical pose");
                    Require(Seen.Count >= 8, "Animation did not advance through sufficient frames");
        ...
        static void Finish(int code)
        {
            ...
            Debug.Log($"ESSA198_PLAYTEST_EXIT={code}");
