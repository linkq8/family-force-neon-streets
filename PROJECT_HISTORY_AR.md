...
آخر تحديث: 9 سبتمبر 2026 — Codex

## حالة العمل الحالية

- أحدث201: نُشر الإصلاح البرمجي في unity-v0.5.2-essa-motion. المشغل يستأنف
  دورة المشي بعد الضرر وانقطاع اللمس. فحص فيديو Android رصد الإطارات1–12
  مرئية، والسجل أثبت11→hurt→11→12 بدل إعادة0. الرسم198 محفوظ كما هو؛
  الصقل الفني للوضعيات لم يكتمل، والمحاولات الجديدة لم تدخل الإصدار.

- أحدث200: اكتمل التشخيص — المستخدم قبل جودة الرسم ورفض جودة التحريك.
  المشي يعرض 12 صورة في اختبار Unity السابق، لكن الوضعيات متقاربة وتفتقر
  إلى انتقالات مقنعة؛ اللكمة 3 وضعيات والركلة وضعيتان والوقوف صورة واحدة.
  لا إثبات لعرض كل إطار على جهاز المستخدم. لم يتغير الرسم أو الكود أو APK.
...
### 2026-09-09-201 — إصلاح تحريك عيسى

- المنفذ: Codex.
- طلب المستخدم: حل المشكلة، بعد تشخيص ضعف أطوار المشي وتكرار مفاتيح القتال.
- الحالة: إصلاح برمجي مختبر ومنشور؛ الصقل الفني للوضعيات لم يكتمل.
- نقطة البداية: 41b4d2d؛ الإصدار السابق unity-v0.5.1-essa-retro.
- ما تم:6 محاولات ImageGen بالمراجع الأصلية وقفل الحركة؛ لم تختر للإصدار
  بسبب تكرار الطرف القائد، وذراع زائد فيquarter2/01. تجربة مفاصل من الرسم198
  تحافظ على الوجه لكنها تشوه الدرع؛ محفوظة ولم تختر للإصدار النهائي.
  اختبار Android للمرشح كشف أن الضرر يعيد المشي إلى0 بعد القطع، ويفقد النصف
  الثاني من الدورة إذا تكرر. أصلح SpriteStripAnimator حفظ واستئناف walkFrame
  وaccumulator عبر الضرر/الحركات/انقطاع اللمس، وتهيئة الحالة وتوقيت الضربات.
  PlayerMotor يوقف المشي عند الحد ويضبط معدل العرض حسب سرعة الانتقال.
- تصحيح مؤرخ لتشخيص200: تكرار الصور مشكلة فنية حقيقية، لكن المشغل أيضًا
  كان يعيد الدورة بعد الضرر. اختبار198 الهادئ لم يكشف ذلك، وPASS لا ينفيه.
- الملفات المعدلة: Runtime/SpriteStripAnimator.cs وPlayerMotor.cs؛
  Editor/BuildFamilyForce.cs؛Editor/PracticalRetroQA/Essa201Playtest.cs و
  Essa201AnimatorRegression.cs وmeta؛unity/tools/test_essa_motion_apk.py و
  inspect_motion_video.swift؛unity/RELEASE_0.5.2_AR.md؛ProjectSettings.asset؛
  art-pipeline/prepare_essa*201.py وtest_motion_201.py والمراجعات والمرشحات201؛
  PROJECT_HISTORY_AR.md. الرسم الأصلي ومصدر PracticalEssaClips محفوظان دون تغيير.
- الاختبارات:33 اختبارًا محليًا PASS؛ اختبار انحدار استئناف المشي عبر الضرر
  وانقطاع اللمس ووصول12 إطارًا تحت المقاطعات PASS؛ اختبار Unity الفعلي PASS
  (12 إطارًا، اتجاهان، وقوف، ضربات، hurt/down/jump). APK النهائية بُنيت وثُبتت
  وشُغلت على Android34/ARM64 محاكي. OCR لفيديوها android-final.mp4 رصد1–12
  جميعها مرئية، لا مجرد سجلات ApplyFrame؛ visible-frames-final.json يحفظ
  التوقيت والقراءة. android-final.log أثبت11→hurt→11→12 واستئناف اللمس عند5
  بدل0. لا Unity errors/FATAL في عملية التطبيق خلال الفحص، ولا جهاز حقيقي.
  فشل البناء الأول لنسخ cpp مكررة بلاحقة2 داخل Gradle cache؛ تطابقت بصماتها،
  فحُفظ المجلد كاملًا في unity/Builds/EssaMotion201/Gradle-duplicate-cache ثم
  أُعيد توليده بنجاح. لا حذف لأصول أو صور أو كود المستخدم.
- Release: APK منشورة Pre-release بالوسم unity-v0.5.2-essa-motion؛ النسخة
  0.5.2-essa-motion-fix،code3،ARMv7/ARM64،حجم54662086بايت،توقيعdebug
  مطابق للإصدار السابق. SHA256:
  `342d56f1231c0ec53e870bee0990f94d6e2e5e79203e73ad47ffcd40b926f61b`.
  المسار unity/Builds/Android/FamilyForceUnity-EssaMotion-0.5.2.apk.
  أدلة البناء والفحص والفيديو تحت unity/Builds/EssaMotion201؛ بياناتUnity
  تحت art-pipeline/builds/Essa/motion-201.
  commit المصدر: `9bb58cce741501e88324b38c14f4350bfa9a8d5e`.
  https://github.com/linkq8/family-force-neon-streets/releases/tag/unity-v0.5.2-essa-motion
  رابط APK المباشر:
  https://github.com/linkq8/family-force-neon-streets/releases/download/unity-v0.5.2-essa-motion/FamilyForceUnity-EssaMotion-0.5.2.apk
  تحقق GitHub: draft=false،state=uploaded،حجم54662086 وبصمةSHA256 مطابقان
  للمحلي. الإصدار0.5.1 محفوظ؛ أُغلق محاكي الاختبار الخاص بهذه الجولة.
- ملاحظات/مخاطر: قبول جودة الصورة لا يعني اعتماد أي وجه جديد؛ الأولوية
  لأطوار متمايزة ومراجعة حلقة كاملة، لا عد بصمات الصور فقط.
- التالي: اختبار المستخدم على جهازه؛ تحريك مفاتيح جسم متمايزة
  يبقى عملًا فنيًا مستقلًا، لا تدّعي هذه النسخة أنه اكتمل.

### 2026-09-09-200 — تشخيص ملاحظة الحركة الجامدة

- المنفذ: Codex.
- طلب المستخدم: كواليتيالصورةممتازةلكنالتحريكجامد؛هلالإطاراتمتطابقةأولا
  تُعرضالإطاراتالاثناعشر؟
- الحالة: اكتمل التشخيص؛ لم يُنفذ إصلاح أو بناء APK جديد في هذا الطلب.
- نقطة البداية:41b4d2d؛الإصدارunity-v0.5.1-essa-retroوالمصدر71d9214.
- ما تم: مراجعة PracticalEssaClips.cs وSpriteStripAnimator.cs وصورة
  art-pipeline/builds/Essa/practical-198/contact-dark.jpg ونتيجة unity-test/result.json.
  المشي يحتوي 12 خلية مستقلة، لكن تقارب وضعيات الرجلين ونقص الأطوار الوسيطة
  يفسران ضعف الحركة بصريًا. ترتيبها وحده لا ينشئ الوضعيات الناقصة.
  اللكمة Hold(1,2,2,0): ثلاث وضعيات؛ الركلة Hold(0,3,3,0): وضعيتان؛
  الوقوف صورة واحدة، والقفز يستخدمها مع إزاحة رأسية من PlayerMotor.
  سرعة المشغل 12fps لا تعني وجود 12 رسمة مختلفة لكل حركة.
  SetMoving لا يعيد الإطار صفرًا كل Update، بل عند تغير حالة الحركة فقط.
- الملفات المعدلة: PROJECT_HISTORY_AR.md فقط.
- الاختبارات: مراجعة نتيجة اختبار198 السابق: seen_walk_frames=12 داخل Unity؛
  لا اختبار جديد ولا قياس إطار بإطار على جهاز المستخدم. الاختبارات السابقة
  أثبتت تبديل الصور والاستجابة، ولم تثبت جودة الوضعيات أو سلاسة الانتقالات.
- Release: لايوجدجديد؛0.5.1يبقىمحفوظًا.
- ملاحظات/مخاطر: عدالإطاراتوالبصماتلايثبتاختلافالوضعياتأوسلاسةالحركة.
- التالي: الحفاظ على جودة الرسم وإنتاج أطوار حركة متمايزة فعلًا، بدءًا بالمشي
  (ملامسة، تحميل الوزن، مرور القدم، رفعها بالتناوب)، ثم تمهيد الضربات وارتدادها؛
  تقييم دورة كاملة بصريًا والتحقق بعدّاد إطارات داخل Android قبل إصدار جديد.
...
- أحدث201: الإصلاح البرمجي مختبر على Unity وAndroid؛ استئناف المشي بعد
  الضرر وانقطاع اللمس بدل إعادة أول إطار. APK النهائية تستخدم رسوم198 الأصلية
  لا مرشح المفاصل. OCR أكد عرض12 إطارًا في فيديو المحاكي. نُشر بالوسم
  unity-v0.5.2-essa-motion ومنcommit9bb58cce؛الرابطوالبصمةفيإدخال201.
  لا اعتماد نهائي لجودة الوضعيات ولا اختبار هاتف حقيقي.

- أحدث200: التشخيص مكتمل دون تغيير الرسم أو الكود أو APK. 12 إطار مشي
  لوحظت في اختبار Unity السابق، لكن الأطوار متقاربة؛ القتال يستخدم مفاتيح
  قليلة مكررة والوقوف ثابت. يلزم إصلاح محتوى الحركة لا زيادة fps وحدها.
  المستخدم قبل جودة الرسم فقط؛ نجاح اختبار المشغل ليس اعتمادًا فنيًا للتحريك.
...
- آخر Release لـUnity: `unity-v0.5.2-essa-motion`، APK SHA-256
  `342d56f1231c0ec53e870bee0990f94d6e2e5e79203e73ad47ffcd40b926f61b`،
  مبني من commit `9bb58cce741501e88324b38c14f4350bfa9a8d5e`؛
  النسختان0.5.0و0.5.1محفوظتان،ورسوم198لمتتغيرفيالإصدارالجديد.
